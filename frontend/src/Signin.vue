...
<div class="row">
   ...
   <div class="signup">
    <input type="text" placeholder="Name" class="inp" v-model="name" />
     <input type="email" placeholder="Email" class="inp" v-model="email" />

     <button class="btn" @click="onSignin">Login</button>
   </div>

   <p class="err" v-if="err">Please enter required field value</p>
</div>
...
export default {
     ...
     methods:{
       onSignin(){
         ...
         var namedata;
         console.log(pay);
         this.$http.post('http://localhost:3000/users',pay)
           .then((user)=>{
             console.log('vue response');
             console.log(user.body.createdUser);
             idtoken = user.body.createdUser._id;
             namedata = user.body.createdUser.name;
